...
function openDialog(DialogComponent, props) {
  ...
  (dialog.props as any).onOk = closeDialog;
  (dialog.props as any).onCancel = dismissDialog;
  (dialog.props as any).afterClose = destroyDialog;
  (dialog as any).close = closeDialog;
  (dialog as any).dismiss = dismissDialog;
  ...
  const result = {
    ...
    result: new DialogWrapper.Promise((resolve, reject) => {
      (dialogResult as any).resolve = resolve;
  ...
  if (isFunction((result.result as any).suppressUnhandledRejections)) {
    (result.result as any).suppressUnhandledRejections();
  }
...
(DialogWrapper as any).wrap = wrap;
