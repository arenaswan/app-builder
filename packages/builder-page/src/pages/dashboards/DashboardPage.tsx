...
function DashboardComponent(props) {
  ...
  return (
    <div className="container" ref={setPageContainer} data-test={`DashboardId${dashboard.id}Container`}>
      ...
      {editingLayout && (
        <AddWidgetContainer className="" dashboardConfiguration={dashboardConfiguration} style={bottomPanelStyles} />
      )}
    </div>
  );
}
...
function DashboardPage({ pageIdSlug, pageId, onError }) {
  const [dashboard, setDashboard] = useState(null);
  const handleError = useImmutableCallback(onError);

  useEffect(() => {
    (Dashboard as any).get({ id: pageId, slug: pageIdSlug })
      .then(dashboardData => {
