...
class DynamicDateRangePicker extends React.Component {
  ...
  dateRangeComponentRef: React.RefObject<unknown>;
  ...
  onDynamicValueSelect = dynamicValue => {
    const { onSelect, parameter } = this.props as any;
    if (dynamicValue === "static") {
      const parameterValue: any = parameter.getExecutionValue();
    ...
    (this.dateRangeComponentRef.current as any).focus();
  };

  render() {
    const { type, value, onSelect, className, dynamicButtonOptions, dateRangeOptions, parameter, ...rest } = this.props as any;
