...
export default function QuerySelector(props) {
      ...
      <ul className="list-group">
        {searchResults.map(q => (
          <PlainButton
            ...
            {q.name} <QueryTagsControl isArchived={false} isDraft={q.is_draft} tags={q.tags} className="inline-tags-control" />
          </PlainButton>
        ))}
      </ul>
    );
  ...
  return (
    <span data-test="QuerySelector">
      {selectedQuery ? (
        <Input value={(selectedQuery as any).name} aria-label="Tied query" suffix={clearIcon} readOnly />
      ) : (
        <Input
          placeholder={placeholder}
          value={searchTerm}
