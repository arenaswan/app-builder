        ...
        <div className="row">
          {map(filters, filter => {
            const options = map(filter.values, (value, index) => (
              <Select.Option key={index} value={index}>{formatColumnValue(value, get(filter, "column.type"))}</Select.Option>
            ));
              ...
              <div
                ...
                {options.length > 0 && (
                  <Select
                    labelInValue
                    className="w-100"
                    mode={filter.multiple ? "multiple" : "tags"}
                    value={
                      ...
                      : { key: `${indexOf(filter.values, filter.current)}`, label: formatColumnValue(filter.current) }) as any)
                    }
                    ...
                    {filter.multiple && [
                      <Select.Option key={NONE_VALUES} value={NONE_VALUES} data-test="ClearOption">
                        <i className="fa fa-square-o m-r-5" aria-hidden="true" />
                        Clear
                      </Select.Option>,
                      <Select.Option key={ALL_VALUES} value={ALL_VALUES} data-test="SelectAllOption">
