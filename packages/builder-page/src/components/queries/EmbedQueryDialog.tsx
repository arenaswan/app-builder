...
class EmbedQueryDialog extends React.Component {
  ...
  constructor(props) {
    ...
    (this as any).embedUrl = `${clientConfig.basePath}embed/query/${query.id}/visualization/${visualization.id}?api_key=${
      query.api_key
    }&${query.getParameters().toUrlParams()}`;

    if ((window as any).snapshotUrlBuilder) {
      (this as any).snapshotUrl = (window as any).snapshotUrlBuilder(query, visualization);
    }
  }
  ...
  render() {
    const { query, dialog } = this.props as any;
          ...
          <React.Fragment>
            ...
            <div>
              <CodeBlock aria-labelledby={this.iframeEmbedLabelId} copyable>
                {`<iframe src="${(this as any).embedUrl}" width="${iframeWidth}" height="${iframeHeight}"></iframe>`}
              </CodeBlock>
            ...
            {(this as any).snapshotUrl && (
              <React.Fragment>
                <h5>Image Embed</h5>
                <CodeBlock copyable>{(this as any).snapshotUrl}</CodeBlock>
              </React.Fragment>
            )}
          </React.Fragment>
        ) : (
