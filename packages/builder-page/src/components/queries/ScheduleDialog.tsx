...
const RadioGroup: any = Radio.Group
...
class ScheduleDialog extends React.Component {
  ...
  getState() {
    const newSchedule = clone((this.props as any).schedule || ScheduleDialog.defaultProps.schedule);
  ...
  get intervals() {
    ...
    (this.props as any).refreshOptions.forEach(seconds => {
  ...
  setInterval = newSeconds => {
    ...
    if (
      ...
      newSchedule.time = (moment() as any)
        .hour("00")
        .minute("15")
        .utc()
        .format(HOUR_FORMAT);
  ...
  save() {
    ...
    const hasChanged = () => {
      ...
      const oldCompact = omitBy((this.props as any).schedule, isNil);
    ...
    if (hasChanged()) {
      if (newSchedule.interval) {
        (this.props as any).dialog.close(clone(newSchedule));
      } else {
        (this.props as any).dialog.close(null);
      }
    }
    (this.props as any).dialog.dismiss();
  }
          ...
          <div data-testid="interval">
            <Select className="input" value={seconds} onChange={this.setInterval} dropdownMatchSelectWidth={false}>
              ...
              {Object.keys(this.intervals)
                ...
                .map(int => (
                  <OptGroup label={capitalize(pluralize(int, undefined))} key={int}>
        ...
        {IntervalEnum.WEEKS === interval ? (
          <div className="schedule-component">
            ...
            <div data-testid="weekday">
              <RadioGroup size="medium" defaultValue={this.state.dayOfWeek} onChange={this.setWeekday}>
        ...
        {interval !== IntervalEnum.NEVER ? (
          <div className="schedule-component">
            ...
            <div className="ends" data-testid="ends">
              <RadioGroup size="medium" value={!!until} onChange={this.setUntilToggle}>
