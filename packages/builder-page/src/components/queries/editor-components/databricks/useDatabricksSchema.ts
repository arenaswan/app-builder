...
function getDatabases(dataSource, refresh = false) {
  ...
  return DatabricksDataSource.getDatabases(dataSource, refresh).catch(() => {
    (notification as any).error("Failed to load Database list.", "Please try again later.");
...
function getSchema(dataSource, databaseName, refresh = false) {
  ...
  return DatabricksDataSource.getDatabaseTables(dataSource, databaseName, refresh).catch(() => {
    (notification as any).error(`Failed to load tables for ${databaseName}.`, "Please try again later.");
    return Promise.reject();
  });
}
  ...
  const setCurrentSchema = useCallback(
    schema =>
      setSchemas(currentSchemas => ({
        ...currentSchemas,
        [(currentDatabaseName as any)]: schema,
      })),
    [currentDatabaseName]
  );
  ...
  const loadTableColumns = useCallback(
    tableName => {
      ...
      DatabricksDataSource.getTableColumns(
        dataSource,
        currentDatabaseName,
        tableName.substring((currentDatabaseName as any).length + 1)
      ).then(columns => {
        if (currentDatabaseNameRef.current === currentDatabaseName) {
          setSchemas(currentSchemas => {
            ...
            return {
              ...currentSchemas,
              [(currentDatabaseName as any)]: updatedSchema,
            };
          });
        }
      });
