...
export class ParameterMappingInput extends React.Component {
  ...
  updateSourceType = type => {
    ...
    const { existingParamNames } = this.props as any;
  ...
  updateParamMapping = update => {
    const { onChange, mapping } = this.props as any;
  ...
  renderDashboardMapToExisting() {
    const { mapping, existingParamNames } = this.props as any;
  ...
  renderStaticValue() {
    const { mapping } = this.props as any;
    return (
      <ParameterValueInput
  ...
  render() {
    const { inputError } = this.props as any;
...
class MappingEditor extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      visible: false,
      mapping: clone((this.props as any).mapping),
      inputError: null,
    };
  }
  ...
  onChange = mapping => {
    ...
    if (mapping.type === MappingType.DashboardAddNew) {
      ...
      } else if (includes((this.props as any).existingParamNames, mapping.mapTo)) {
  ...
  save = () => {
    (this.props as any).onChange((this.props as any).mapping, (this.state as any).mapping);
    this.hide();
  };
  ...
  show = () => {
    this.setState({
      visible: true,
      mapping: clone((this.props as any).mapping), // restore original state
    });
  };
  ...
  renderContent() {
    const { mapping, inputError } = this.state as any;
    ...
    return (
      <div className="parameter-mapping-editor" data-test="EditParamMappingPopover">
        ...
        <ParameterMappingInput
          mapping={mapping}
          existingParamNames={(this.props as any).existingParamNames}
  ...
  render() {
    const { visible, mapping } = this.state as any;
    return (
      <Popover
        placement="left"
        trigger="click"
...
class TitleEditor extends React.Component {
  ...
  getMappingTitle() {
    ...
    if (mapping.type === MappingType.DashboardMapToExisting) {
      const source = find((this.props as any).existingParams, { name: mapping.mapTo });
      if (source) {
        mapping = source;
      }
    }
  ...
  save = () => {
    const newMapping = extend({}, (this.props as any).mapping, { title: (this.state as any).title });
    (this.props as any).onChange(newMapping);
    this.hide();
  };
  ...
  renderPopover() {
    ...
    } = (this.props as any).mapping;

    return (
      <div className="parameter-mapping-title-editor">
        <Input
          size="small"
          value={(this.state as any).title}
  ...
  renderEditButton() {
    ...
    return (
      <Popover
        ...
        visible={(this.state as any).showPopup}
...
export class ParameterMappingListInput extends React.Component {
  ...
  updateParamMapping(oldMapping, newMapping) {
    const mappings = [...(this.props as any).mappings];
    ...
    (this.props as any).onChange(mappings);
  }

  render() {
    const { existingParams } = this.props as any; // eslint-disable-line react/prop-types
    const dataSource = (this.props as any).mappings.map(mapping => ({ mapping }));
      ...
      <div className="parameters-mapping-list">
        <Table dataSource={dataSource} size="middle" pagination={false} rowKey={(record, idx) => `row${idx}`}>
          ...
          <Table.Column
            ...
            dataIndex="mapping"
            key="value"
            render={mapping => (this.constructor as any).getDefaultValue(mapping, (this.props as any).existingParams)}
          />
          <Table.Column
            title="Value Source"
            dataIndex="mapping"
